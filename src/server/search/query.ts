 ...
 * El índice GiST en `products.normalized_name` hace que `%` sea rápido.
 * Los parámetros se binden vía `Prisma.sql` templates — no `$N` posicional.
 */
  ...
  const rows = await prisma.$queryRaw<RankedRow[]>(Prisma.sql`
    WITH candidates AS (
      SELECT
        ...
        similarity(p.normalized_name, ${normalized}) AS name_sim,
        similarity(COALESCE(LOWER(p.brand), ''), ${normalized}) AS brand_sim,
        (
      ...
      WHERE p.canonical_id IS NULL
        AND (
          ...
          OR COALESCE(LOWER(p.brand), '') % ${normalized}
        )
    ),
    scored AS (
      SELECT *,
